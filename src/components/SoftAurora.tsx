...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      if (program) {
        (program.uniforms.uResolution as any).value = [
          gl.canvas.width,
          gl.canvas.height,
    ...
    function update(time: number) {
      ...
      if (!program) return;
      (program.uniforms.uTime as any).value = time * 0.001;
  ...
  return (
    <div
      ref={containerRef}
      className={`soft-aurora-container absolute inset-0 h-full w-full overflow-hidden ${className ?? ""}`}
    />
  );
}
